import { useSelector } from "react-redux"; //useSelector is a hook provided by React Redux that allows components to extract data from the Redux store state.
import { getCartItems, getTotals } from "../../Redux/cartSlice"; // getCartItems and getTotals are functions defined in cartSlice that select specific data from the Redux store.
import CartItem from "./CartItem"; // CartItem is a component used to render individual items in the cart.
import { useDispatch } from 'react-redux'; // useDispatch is a hook provided by React Redux to dispatch actions to the Redux store.
import { useEffect } from "react"; // useEffect and useState are React hooks used to handle side effects and manage component state, respectively.
import { useState } from "react";

const Cart = () => {
const cart = useSelector((state) => state.cart); // useSelector to extract data from the Redux store
const cartItems = useSelector(getCartItems) // Selecting specific data (cartItems) from the 'cart' state using a selector from the Redux slice
const dispatch = useDispatch(); // Accessing the Redux dispatch function to dispatch actions
const [showMore, setShowMore] = useState(false); // State for managing the visibility of additional information (showMore)

  useEffect(() => { // useEffect hook to dispatch the 'getTotals' action when the 'cart' state or dispatch function changes
    dispatch(getTotals());
      }, [cart, dispatch]);
  ...
  return (<div className="cart">
    <h4>Your Cart</h4>
    {/* Rendering each cart item using the 'CartItem' component */}
      {cart.cartItems.length === 0 ? (  
        ...
        <div className="aboutTotal">
          {/* Displaying the total quantity and total amount of items in the cart */}
        <h3>Item(s): {cart.cartTotalQuantity}</h3>
            <h3>Total: $ {cart.cartTotalAmount}</h3>
            <div>
            <button className="btnEtsy"> {/* Button to visit an external link */}
            ...
            {/* Button to toggle the visibility of additional information */}
